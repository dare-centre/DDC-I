...
train_val_y <- select(train_val_y, all_of(c("Date", y_var)))
test_y <- select(test_y, all_of(c("Date", y_var)))
...
cat("Number of features: ", ncol(train_val_x[, -1]),    # Allow for date column.
...
colnames(test_x_all)[-1]

cat("\nPredict variable: ", paste0('"', colnames(train_val_y)[-1], '"  '),
  sep = "")
...
if (!is.null(best_n)) {
    ...
    #
    # enforce equal table lengths
    rows_diff <- nrow(train_x_all) - nrow(train_val_y)
    if (rows_diff > 1) {
      train_val_y <- append_rows(train_val_y, rows_diff)
    } else if (rows_diff < 1) {
      train_x_all <- append_rows(train_x_all, abs(rows_diff))
    }
    #
    # select the top n features, drop the observed variable
    corr_mtrx <- head(abs(psych::corr.test(x = bind_cols(select(train_x_all, -1), select(train_val_y, -1)))$r), -1)
    ...
    # use all features
    top_n_features <- colnames(train_x_all)[-1]
}

# Now
train_x <- train_x_all[top_n_features]
train_time <- train_x_all[, 1]
test_x <- test_x_all[top_n_features]
test_time <- test_x_all[, 1]
if (val_split) {
  val_x <- val_x_all[top_n_features]
  val_time <- val_x_all[, 1]
